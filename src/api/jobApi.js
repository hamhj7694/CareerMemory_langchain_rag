...
export const jobApi = {
  ...
  extractFiles(files, options = {}) {
    ...
    return apiRequest({ path: '/api/jobs/extract-text', method: 'POST', body, ...options });
  },
  ...
  rematch(jobId, requirementIds = [], options = {}) { const path = `/api/jobs/${idPath(jobId)}/match`; return apiMode === 'mock' && !hasMockHandler('POST', path) ? unifiedMockApi.matchJob(jobId, { requirementIds }) : apiRequest({ path, method: 'POST', body: { requirementIds, refresh: true, clientRequestId: createClientRequestId() }, ...options }); },
