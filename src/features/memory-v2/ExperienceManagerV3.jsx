...
function EditHistoryButton({ type, disabled, onClick }) {
  const undo = type === 'undo';
  const label = undo ? '뒤로 되돌리기' : '다시 실행';
  return <button type="button" className="mv2-edit-history-button" disabled={disabled} onClick={onClick} aria-label={label} title={label}>
    <svg viewBox="0 0 24 24" aria-hidden="true">
      {undo ? <><path d="M9 7 4 12l5 5" /><path d="M5 12h8a6 6 0 0 1 6 6" /></> : <><path d="m15 7 5 5-5 5" /><path d="M19 12h-8a6 6 0 0 0-6 6" /></>}
    </svg>
  </button>;
}
...
function sourceEntries(item) {
  return (item.sourceRefs || []).map((source, index) => {
    if (!source || typeof source === 'string') {
      return {
        id: typeof source === 'string' ? source : `${item.id}-missing-source-${index}`,
        type: 'unknown',
        name: '원본 정보 없음',
        unavailable: true,
      };
    }
    const sourceType = source.sourceType || source.source_type || source.kind;
    const type = sourceType === 'file' || sourceType === 'pdf' || source.filename ? 'file' : 'text';
    return {
      ...source,
      id: source.id || source.source_id || `${item.id}-source-${index}`,
      type,
      name: source.filename || source.title || (type === 'text' ? '텍스트 입력' : '첨부 파일'),
      content: source.text || '',
      mimeType: source.mimeType || source.mime_type || (type === 'text' ? 'text/plain;charset=utf-8' : 'application/octet-stream'),
      rawBytes: source.rawBytes || source.raw_bytes || null,
      uploadedAt: source.uploadedAt || source.uploaded_at || source.capturedAt || source.captured_at || '',
      unavailable: type === 'file' && !source.rawBytes && !source.raw_bytes && !source.text,
    };
  });
}

function downloadSource(source) {
  const payload = source.rawBytes || source.content;
  if (!payload) return;
  const url = URL.createObjectURL(new Blob([payload], { type: source.mimeType || 'application/octet-stream' }));
...
export function ExperienceManagerV3() {
  ...
  const captureEditState = (overrides = {}) => structuredClone({
    domains,
    experiences,
    structureOrder,
    pendingOps,
  ...
  const restoreEditState = (snapshot) => {
    ...
    setStructureOrder(snapshot.structureOrder);
    setPendingOps(snapshot.pendingOps);
    setDraftProject(snapshot.draftProject);
    setNameEditor(snapshot.nameEditor);
    setConfirm(null);
    setDropTarget(null);
    setDragging(null);
    setSelectedExperienceId(null);
  };
  const recordEditHistory = (overrides) => {
    const snapshot = captureEditState(overrides);
    setUndoHistory((items) => [...items.slice(-49), snapshot]);
    setRedoHistory([]);
  };
  const undoEdit = () => {
    if (!undoHistory.length || savingStructure) return;
    const previous = undoHistory[undoHistory.length - 1];
    setRedoHistory((items) => [...items.slice(-49), captureEditState()]);
    setUndoHistory((items) => items.slice(0, -1));
    restoreEditState(previous);
  };
  const redoEdit = () => {
    if (!redoHistory.length || savingStructure) return;
    const next = redoHistory[redoHistory.length - 1];
    setUndoHistory((items) => [...items.slice(-49), captureEditState()]);
    setRedoHistory((items) => items.slice(0, -1));
    restoreEditState(next);
  };
  const beginEditMode = () => {
    editSnapshotRef.current = { domains: structuredClone(domains), experiences: structuredClone(experiences), structureOrder: structuredClone(structureOrder) };
    setPendingOps([]);
    setUndoHistory([]);
    setRedoHistory([]);
    setEditMode(true);
  ...
  const cancelEditMode = () => {
    ...
    setPendingOps([]); setUndoHistory([]); setRedoHistory([]); setEditMode(false); setDropTarget(null); setDragging(null); setDraftProject(null); setNameEditor(null); editSnapshotRef.current = null;
  };
  const addDomain = () => {
    recordEditHistory();
    const existingNames = new Set(domains.map((domain) => structureNameKey(domain.name)));
    let name = '새 경험';
    let suffix = 2;
    while (existingNames.has(structureNameKey(name))) {
      name = `새 경험 ${suffix}`;
      suffix += 1;
    }
    const domain = { id: `temp-domain-${crypto.randomUUID()}`, name, version: 1, projects: [], created_at: new Date().toISOString() };
    setQuery('');
    setDomains((items) => [domain, ...items]);
    setPendingOps((items) => [...items, { type: 'create-domain', domain }]);
    setCollapsed((items) => {
  ...
  const addProject = (domain) => {
    if (domain.id.startsWith('temp-domain-')) {
      saveStructure({ openExperienceFor: { domainId: domain.id, projectId: '' } });
      return;
    }
  ...
  const commitNameEditor = () => {
    ...
    recordEditHistory({ nameEditor: null });
  ...
  const requestDeleteDomain = (domain) => setConfirm({ title: `‘${domain.name}’ 분류를 삭제 목록에 추가할까요?`, description: `저장할 때 프로젝트·활동 ${domain.projects.length}개와 포함된 경험이 휴지통으로 이동합니다. 저장 전에는 취소할 수 있습니다.`, action: () => { recordEditHistory(); setDomains((items) => items.filter((item) => item.id !== domain.id)); setExperiences((items) => items.filter((item) => item.domainId !== domain.id)); setPendingOps((items) => [...items, { type: 'delete-domain', domain }]); setConfirm(null); } });
  const requestDeleteProject = (project) => setConfirm({ title: `‘${project.name}’을 삭제 목록에 추가할까요?`, description: '저장할 때 포함된 경험이 휴지통으로 이동합니다. 저장 전에는 취소할 수 있습니다.', action: () => { recordEditHistory(); setDomains((items) => items.map((domain) => ({ ...domain, projects: domain.projects.filter((item) => item.id !== project.id) }))); setExperiences((items) => items.filter((item) => item.projectId !== project.id)); setPendingOps((items) => [...items, { type: 'delete-project', project }]); setConfirm(null); } });
  const requestDeleteExperience = (experience) => setConfirm({ title: `‘${experience.title}’ 경험을 삭제 목록에 추가할까요?`, description: '저장할 때 경험이 휴지통으로 이동합니다. 저장 전에는 취소할 수 있습니다.', action: () => { recordEditHistory(); setExperiences((items) => items.filter((item) => item.id !== experience.id)); setPendingOps((items) => [...items, { type: 'delete-experience', experience }]); setConfirm(null); if (selected?.id === experience.id) setSelectedExperienceId(null); } });
  const addReorderOperation = (type) => setPendingOps((items) => [...items.filter((item) => item.type !== type), { type }]);
  const moveIdRelative = (ids, sourceId, targetId, position = 'before') => {
    const next = ids.filter((id) => id !== sourceId);
    const targetIndex = next.indexOf(targetId);
    const insertIndex = targetIndex < 0 ? next.length : targetIndex + (position === 'after' ? 1 : 0);
    next.splice(insertIndex, 0, sourceId);
    return next;
  };
  const sameOrder = (left, right) => left.length === right.length && left.every((id, index) => id === right[index]);
  const dropPosition = (event, element = event.currentTarget) => {
    const rect = element.getBoundingClientRect();
    return event.clientY < rect.top + rect.height / 2 ? 'before' : 'after';
  };
  const showDropTarget = (kind, id, position) => {
    setDropTarget((current) => current?.kind === kind && current?.id === id && current?.position === position ? current : { kind, id, position });
  };
  const leaveDropTarget = (event, id, kinds) => {
    if (event.currentTarget.contains(event.relatedTarget)) return;
    setDropTarget((current) => current?.id === id && kinds.includes(current.kind) ? null : current);
  };
  const dropIndicatorClass = (kind, id) => dropTarget?.kind === kind && dropTarget?.id === id ? `is-drop-${dropTarget.position}` : '';
  const draggingClass = (kind, id) => dragging?.kind === kind && dragging?.id === id ? 'is-dragging' : '';
  const clearDragState = () => {
    setDropTarget(null);
    setDragging(null);
  };
  const reorderDomain = (sourceId, targetId, position = 'before') => {
    if (!editMode || sourceId === targetId) return;
    const ids = sortBySavedOrder(domains, structureOrder.domains).map((domain) => domain.id);
    const nextIds = moveIdRelative(ids, sourceId, targetId, position);
    if (sameOrder(ids, nextIds)) return;
    recordEditHistory();
    setStructureOrder((value) => ({ ...value, domains: nextIds }));
    addReorderOperation('reorder-domains');
  };
  const reorderProject = (domainId, sourceId, targetId, position = 'before') => {
    if (!editMode || sourceId === targetId) return;
    ...
    if (sameOrder(ids, nextIds)) return;
    recordEditHistory();
    setStructureOrder((value) => ({ ...value, projects: { ...value.projects, [domainId]: nextIds } }));
    addReorderOperation(`reorder-projects:${domainId}`);
  };
  const moveProject = (projectId, targetDomainId, targetProjectId = '', position = 'after') => {
    if (!editMode || !projectId || !targetDomainId) return;
    const sourceDomain = domains.find((domain) => domain.projects.some((project) => project.id === projectId));
    const targetDomain = domains.find((domain) => domain.id === targetDomainId);
    const project = sourceDomain?.projects.find((item) => item.id === projectId);
    if (!sourceDomain || !targetDomain || !project) return;
    if (sourceDomain.id === targetDomain.id && targetProjectId) {
      reorderProject(targetDomain.id, projectId, targetProjectId, position);
      return;
    }
    if (sourceDomain.id === targetDomain.id) {
      const ids = sortBySavedOrder(sourceDomain.projects, structureOrder.projects[sourceDomain.id]).map((item) => item.id);
      const nextIds = [...ids.filter((id) => id !== projectId), projectId];
      if (sameOrder(ids, nextIds)) return;
      recordEditHistory();
      setStructureOrder((value) => ({ ...value, projects: { ...value.projects, [sourceDomain.id]: nextIds } }));
      addReorderOperation(`reorder-projects:${sourceDomain.id}`);
      setDropTarget(null);
      return;
    }
    const duplicate = targetDomain.projects.some((item) => item.id !== projectId && structureNameKey(item.name) === structureNameKey(project.name));
    if (duplicate) {
      setError(`"${targetDomain.name}"에 같은 이름의 프로젝트·활동이 있어 이동할 수 없습니다.`);
      return;
    }
    const sourceIds = sortBySavedOrder(sourceDomain.projects, structureOrder.projects[sourceDomain.id]).map((item) => item.id);
    const targetIds = sortBySavedOrder(targetDomain.projects, structureOrder.projects[targetDomain.id]).map((item) => item.id);
    const nextTargetIds = targetProjectId
      ? moveIdRelative(targetIds, projectId, targetProjectId, position)
      : [...targetIds.filter((id) => id !== projectId), projectId];
    if (sourceDomain.id === targetDomain.id && sameOrder(sourceIds, nextTargetIds)) return;

    recordEditHistory();
    setDomains((items) => items.map((domain) => {
      if (domain.id === sourceDomain.id && domain.id !== targetDomain.id) {
        return { ...domain, projects: domain.projects.filter((item) => item.id !== projectId) };
      }
      if (domain.id === targetDomain.id) {
        const remaining = domain.projects.filter((item) => item.id !== projectId);
        return { ...domain, projects: [...remaining, { ...project, domain_id: targetDomain.id }] };
      }
      return domain;
    }));
    setExperiences((items) => items.map((item) => item.projectId === projectId ? {
      ...item,
    ...
    setStructureOrder((value) => {
      const projects = Object.fromEntries(
        Object.entries(value.projects).map(([domainId, ids]) => [domainId, ids.filter((id) => id !== projectId)]),
      );
      projects[sourceDomain.id] = (projects[sourceDomain.id] || sourceIds).filter((id) => id !== projectId);
      projects[targetDomain.id] = nextTargetIds;
      return { ...value, projects };
    });
    setPendingOps((items) => {
      const next = items
        .filter((item) => !(item.type === 'move-project' && item.projectId === projectId))
        .map((item) => item.type === 'create-project' && item.project.id === projectId
          ? { ...item, domainId: targetDomain.id }
          : item);
      if (!projectId.startsWith('temp-')) {
        next.push({ type: 'move-project', projectId, domainId: targetDomain.id, version: project.version });
      }
      return next;
    });
    setError('');
  ...
  const reorderExperience = (projectId, sourceId, targetId, position = 'before') => {
    if (!editMode || sourceId === targetId) return;
    ...
    if (sameOrder(ids, nextIds)) return;
    recordEditHistory();
    setStructureOrder((value) => ({ ...value, experiences: { ...value.experiences, [projectId]: nextIds } }));
    addReorderOperation(`reorder-experiences:${projectId}`);
  };
  const moveExperience = (experienceId, projectId, targetExperienceId = '', position = 'after') => {
    ...
    setStructureOrder((value) => {
      ...
      const visibleTargetIds = sortBySavedOrder(experiences.filter((item) => item.projectId === projectId && item.id !== experienceId), value.experiences[projectId]).map((item) => item.id);
      nextExperiences[projectId] = targetExperienceId
        ? moveIdRelative(visibleTargetIds, experienceId, targetExperienceId, position)
        : [...visibleTargetIds, experienceId];
  ...
  const saveStructure = async ({ openExperienceFor = null } = {}) => {
    ...
    try {
      ...
      const deletedProjectIds = new Set(pendingOps.filter((item) => item.type === 'delete-project').map((item) => item.project.id));
      const projectUpdates = new Map();
      for (const operation of pendingOps) {
        if (!['rename-project', 'move-project'].includes(operation.type)) continue;
        const projectId = operation.id || operation.projectId;
        if (!projectId || projectId.startsWith('temp-') || deletedProjectIds.has(projectId)) continue;
        const current = projectUpdates.get(projectId) || { base_version: operation.version };
        if (operation.type === 'rename-project') current.name = operation.name;
        if (operation.type === 'move-project') current.domain_id = idMap.get(operation.domainId) || operation.domainId;
        projectUpdates.set(projectId, current);
      }
      for (const [projectId, changes] of projectUpdates) {
        await v2ChatApi.updateProject(projectId, changes);
      }
      for (const operation of pendingOps) {
  ...
  const saveAllExperienceDrafts = async () => {
    ...
    try {
      ...
      const results = await Promise.allSettled(pending.map(({ item }) => saveProposalExperience(item)));
      let nextProposal = current;
       const failures = [];
      results.forEach((result, resultIndex) => {
         const { index } = pending[resultIndex];
        if (result.status === 'fulfilled') nextProposal = markProposalExperienceSaved(nextProposal, index, result.value);
  ...
  return <div className={`mv2-manager mv2-manager--v3 ${selected ? 'has-preview' : ''} ${editMode ? 'is-edit-mode' : ''}`}>
    <main>
      ...
      <div className="mv2-structure-heading"><h2>경험 구조</h2><span>{visibleExperienceCount}개 경험</span></div>
      <div className="mv2-structure">{displayDomains.map(({ domain, projects, experiences: domainExperiences }) => {
        ...
        const domainProjectDropClass = dropTarget?.kind === 'domain-container' && dropTarget.id === domain.id ? 'is-project-drop-target' : '';
        return <section key={domain.id} className={`${isCollapsed ? 'is-collapsed' : ''} ${dropIndicatorClass('domain', domain.id)} ${domainProjectDropClass} ${draggingClass('domain', domain.id)}`} onDragOver={(event) => { if (!editMode || !['domain', 'project'].includes(dragging?.kind)) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; if (dragging.kind === 'domain') { if (dragging.id === domain.id) { setDropTarget(null); return; } const header = event.currentTarget.querySelector(':scope > header'); showDropTarget('domain', domain.id, dropPosition(event, header || event.currentTarget)); } else { showDropTarget('domain-container', domain.id, 'inside'); } }} onDragLeave={(event) => leaveDropTarget(event, domain.id, ['domain', 'domain-container'])} onDrop={(event) => { if (!editMode) return; event.preventDefault(); event.stopPropagation(); const payload = readDragPayload(event); if (payload?.kind === 'domain') reorderDomain(payload.id, domain.id, dropTarget?.position || 'before'); if (payload?.kind === 'project') moveProject(payload.id, domain.id); clearDragState(); }}>
          <header className="mv2-structure-header" draggable={editMode && nameEditor?.id !== domain.id} onDragStart={(event) => { event.stopPropagation(); event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('application/json', JSON.stringify({ kind: 'domain', id: domain.id })); setDragging({ kind: 'domain', id: domain.id }); }} onDragEnd={clearDragState} onClick={toggleDomain}>{nameEditor?.kind === 'domain' && nameEditor.id === domain.id ? renderNameEditor() : <button className="mv2-domain-toggle" aria-expanded={!isCollapsed}><h3><HighlightText text={domain.name} query={query} /></h3></button>}{editMode && nameEditor?.id !== domain.id && <MoreMenu label={`${domain.name} 관리`}><button onClick={() => addProject(domain)}>프로젝트·활동 추가</button><button onClick={() => renameDomain(domain)}>이름 변경</button><button className="is-danger" onClick={() => requestDeleteDomain(domain)}>분류 삭제</button></MoreMenu>}<span className="mv2-domain-count">{domainExperiences.length}개 경험</span><button type="button" className="mv2-domain-chevron" aria-label={`${domain.name} ${isCollapsed ? '펼치기' : '접기'}`} aria-expanded={!isCollapsed}>{isCollapsed ? '⌄' : '⌃'}</button></header>
          {!isCollapsed && <div className="mv2-projects">{projects.map(({ project, experiences: projectExperiences }) => {
    ...
    <ExperienceProposalModal
      open={experiencePreviewOpen}
      proposal={experienceProposal}
      busy={experiencePreviewBusy}
      editing={experienceProposalEditing}
      pendingCount={pendingExperienceCount}
      onClose={closeExperiencePreview}
      onApprove={approveExperiencePreview}
      onChange={updateExperiencePreview}
      onRemove={removeExperiencePreview}
      onEditingChange={setExperienceProposalEditing}
      onDiscardRemaining={discardRemainingExperienceDrafts}
      onSaveAll={saveAllExperienceDrafts}
    />
