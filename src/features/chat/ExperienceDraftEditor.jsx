...
export function ExperienceDraftEditor({ item, index, collapseKey, editing, approved, saving, onUpdate, onEdit, onSave, onCancel, onDelete, onApprove }) {
  ...
  return <div className={`v2-draft-structure v2-draft-structure--detail ${editing ? 'is-editing' : ''}`}>
    ...
    {!collapsed && <div className={`v2-draft-project ${projectCollapsed ? 'is-collapsed' : ''}`}>
      ...
      {!projectCollapsed && <article className="v2-draft-detail">
        ...
        <div className="v2-draft-detail-grid">
          <div className="v2-draft-detail-main">
            <section><h4>요약</h4>{editing ? <textarea rows="3" value={item.summary || ''} onChange={(event) => update('summary', event.target.value)} /> : <ExperienceRichText text={item.summary} empty="정리된 요약이 없습니다." />}</section>
            <section><h4>상황</h4>{editing ? <textarea rows="3" value={item.situation || ''} onChange={(event) => update('situation', event.target.value)} /> : <ExperienceRichText text={item.situation} empty="확인된 상황이 없습니다." />}</section>
            <section><h4>행동</h4>{editing ? <textarea rows="4" value={listToText(item.actions)} onChange={(event) => update('actions', textToMarkdownLines(event.target.value))} placeholder="Markdown 목록과 줄바꿈을 사용할 수 있습니다." /> : <ExperienceRichList items={item.actions} empty="확인된 행동이 없습니다." />}</section>
            <section><h4>결과</h4>{editing ? <textarea rows="3" value={listToText(item.results)} onChange={(event) => update('results', textToMarkdownLines(event.target.value))} placeholder="Markdown 목록과 줄바꿈을 사용할 수 있습니다." /> : <ExperienceRichList items={item.results} empty="확인된 결과가 없습니다." />}</section>
          </div>
          <aside>
            <section><h4>내 직군·직업 및 역할</h4>{editing ? <input value={item.role || ''} onChange={(event) => update('role', event.target.value)} /> : <p>{item.role || '확인된 역할이 없습니다.'}</p>}</section>
            ...
            <section><h4>근거에서 확인된 내용</h4>{editing ? <textarea rows="4" value={listToText(item.facts)} onChange={(event) => update('facts', textToMarkdownLines(event.target.value))} placeholder="원본 근거에서 확인된 수치, 기간, 횟수와 성과를 입력해 주세요." /> : <ExperienceRichList items={item.facts} empty="근거에서 확인된 내용이 없습니다." />}</section>
