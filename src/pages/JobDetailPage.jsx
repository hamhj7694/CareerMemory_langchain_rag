...
function RequirementExperienceTree({ experiences, linkedIds, recommendedIds, onToggle, onOpenDetail }) {
  ...
  return <div className="job-experience-structure">{structure.map((domain) => <section key={domain.id} className="job-experience-domain"><header><span>경험 분류</span><strong>{domain.name}</strong><small>{domain.projects.reduce((sum, project) => sum + project.experiences.length, 0)}개 경험</small></header><div className="job-experience-projects">{domain.projects.map((project) => <div className="job-experience-project" key={project.id}><div className="job-experience-project__title"><span>프로젝트·활동</span><strong>{project.name}</strong></div><div className="job-experience-cards">{project.experiences.map((experience) => { const linked = linkedIds.has(experience.experienceId); const recommended = recommendedIds.has(experience.experienceId); return <article className={`job-experience-card ${linked ? 'is-selected' : ''}`} key={experience.experienceId}><label className="job-experience-card__select"><input type="checkbox" checked={linked} onChange={() => onToggle(experience.experienceId)} /><span><strong>{experience.title}</strong><small>{experience.skills.slice(0, 2).join(' · ') || '역량 미입력'}</small></span></label><span className="job-experience-card__badges">{recommended && <em>AI 추천</em>}{linked && !recommended && <em className="is-manual">직접 연결</em>}<button type="button" onClick={() => onOpenDetail(experience)}>상세 보기</button></span></article>; })}</div></div>)}</div></section>)}</div>;
}
...
export function JobDetailPage() {
  ...
  const activeRequirement = job?.requirements?.find((item) => item.id === activeRequirementId) || job?.requirements?.[0];
  ...
  return <section className="feature-page job-result-page">
    <header className="feature-heading split"><div><span className="eyebrow">{job.companyName || '회사 미입력'} · {job.roleName || '직무 미입력'}</span><h1>{job.postingTitle || job.roleName || '채용공고 분석'}</h1><p>{job.requirements?.length || 0}개의 요구사항과 내 경험을 함께 분석합니다.</p></div><div className="header-actions">{job.sourceUrl && <a className="ui-button ui-button--secondary" href={job.sourceUrl} target="_blank" rel="noreferrer">원문 공고 열기</a>}<button className="ui-button ui-button--secondary" onClick={() => navigate('/jobs')}>다른 공고 분석</button></div></header>
    ...
    <div className="job-result-flow">
      <section className="surface job-step-panel requirements-panel"><div className="section-title"><div><span className="step">1</span><div><h2>공고 요구사항</h2><p>카드를 선택하면 연결된 경험을 확인할 수 있습니다.</p></div></div></div>
        ...
        <div className="requirement-list">{job.requirements?.map((requirement, index) => { const isActive = activeRequirement?.id === requirement.id; const connected = (requirementLinks[requirement.id] || []).map((id) => experienceCatalog.find((item) => item.experienceId === id)).filter(Boolean); return <article className={`requirement-card requirement-card--selectable ${isActive ? 'is-focused' : ''}`} key={requirement.id}><button type="button" className="requirement-card__main" onClick={() => { setActiveRequirementId(requirement.id); setExperienceView('recommended'); }} aria-pressed={isActive}><div className="requirement-card__title-row"><span className="requirement-number">{index + 1}</span><h3>{requirement.text}</h3><Tag tone={requirement.importance === 'required' ? 'danger' : 'ai'}>{requirement.importance === 'required' ? '필수' : '우대'}</Tag></div></button><div className={`requirement-linked-experiences ${connected.length ? '' : 'is-empty'}`}>{connected.length ? connected.map((experience) => { const chipId = `${requirement.id}:${experience.experienceId}`; const isOpen = activeLinkedChip === chipId; return <span className={`requirement-linked-chip ${isOpen ? 'is-open' : ''}`} key={experience.experienceId}><button type="button" className="requirement-linked-chip__label" onClick={() => setActiveLinkedChip(isOpen ? '' : chipId)} aria-expanded={isOpen}>{experience.title}</button>{isOpen && <span className="requirement-linked-chip__actions"><button type="button" onClick={() => setDetailExperience(experience)} aria-label={`${experience.title} 상세 보기`} title="상세 보기"><SearchIcon /></button><button type="button" className="is-danger" onClick={() => unlinkRequirementExperience(requirement.id, experience.experienceId)} aria-label={`${experience.title} 연결 해제`} title="연결 해제"><TrashIcon /></button></span>}</span>; }) : <span className="requirement-linked-empty">연결된 경험이 없습니다.</span>}</div></article>; })}</div>
      </section>

      <section className="surface job-step-panel match-panel"><div className="section-title"><div><span className="step">2</span><div><h2>요구사항별 매칭 경험</h2><p>{activeRequirement ? `“${activeRequirement.text}”에 연결된 경험입니다.` : '요구사항을 선택해 주세요.'}</p></div></div></div>
