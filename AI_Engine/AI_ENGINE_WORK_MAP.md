...
| AI-QA-530 | `[ ]` | 경험 추출 정답셋·자동 평가 | 0개·1개·복수 경험, 정보 부족, 텍스트·PDF·이미지 조합의 정답셋을 버전 관리하고 경험 분리 Precision/Recall/F1과 필드 정확도를 자동 산출 |
| AI-QA-540 | `[ ]` | RAG·인용 평가 및 튜닝 | 요구사항·질문별 Recall@K, Precision@K, MRR, 인용 정확도·완전성을 측정하고 K·유사도 임계값·chunk 크기·overlap을 평가 결과로 조정 |
| AI-QA-550 | `[ ]` | 라우터 평가 | 자동 모드 혼합 의도와 낮은 확신 fallback 검증 |
| AI-QA-560 | `[ ]` | API 계약 테스트 | snake_case ↔ camelCase mapper, 오류, SSE 재연결 검증 |
| AI-QA-570 | `[ ]` | 프론트엔드 E2E | Playwright 등으로 채팅 → 초안 검토·개별/전체 저장 → 경험 관리 → 공고 매칭과 새로고침·실패 복구까지 회귀 통과 |
| AI-QA-580 | `[ ]` | AI 성능·비용 관측 | 요청별 P50/P95 지연시간, 입력·출력 토큰, 예상 비용, 오류율을 요청 ID와 모델·Prompt·스키마 버전 기준으로 기록 |
| AI-OPS-610 | `[ ]` | 운영 저장소·장기 작업 처리 | 객체 저장소·운영 DB·rate limit·비동기 작업 queue·재시도·멱등성·상태 조회를 구성하고 부하·장애·복구 검증 |
...
- 프론트엔드 lazy loading·대형 컴포넌트 분리는 전체 백로그 `docs/TODO.md`의 `FUT-FE-PERF`에서 관리한다.
- 위 품질·운영 항목은 추후 업데이트 목록이며, 이번 작업에서는 구현 코드를 변경하지 않는다.
